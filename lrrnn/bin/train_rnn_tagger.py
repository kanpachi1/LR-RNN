...
from lrrnn.taggers import RNNTagger
...
if __name__ == "__main__":
    ...
    char1_features, char1_ttoi = extract_features(
        sentences, extract_char_from_tokens, n_gram=1, window_size=3
    )
    char2_features, char2_ttoi = extract_features(
        sentences, extract_char_from_tokens, n_gram=2, window_size=3
    )
    char3_features, char3_ttoi = extract_features(
        sentences, extract_char_from_tokens, n_gram=3, window_size=3
    )
    type1_features, type1_ttoi = extract_features(
        sentences, extract_chartype_from_tokens, n_gram=1, window_size=3
    )
    type2_features, type2_ttoi = extract_features(
        sentences, extract_chartype_from_tokens, n_gram=2, window_size=3
    )
    type3_features, type3_ttoi = extract_features(
        sentences, extract_chartype_from_tokens, n_gram=3, window_size=3
    )
    word_features, word_ttoi = extract_features(sentences, extract_word_features)

    tags = Tags[config["level"] - 1]
    tag_to_id = data_loader.build_vocab(tags, min_count=1)
    id_to_tag = [t for t in tag_to_id.keys()]
    ...
    sentences, Tags = data_loader.load_corpus_data_for_pos_tagging(config["valid"])
    tags = Tags[config["level"] - 1]
    y = []
    for t in tags:
        id = tag_to_id.get(t)
        if id is None:
            y.append(tag_to_id[info.unk])
        else:
            y.append(id)
    y = np.array(y, dtype=np.int64)
    valid_dl = torch.utils.data.DataLoader(
        data_loader.TaggerEvalDataset(sentences, y, id_to_tag),
        batch_size=1,
        shuffle=False,
    )
    ...
    def create_evaluator():
        ...
        def _inference(engine, batch):
            model.eval()
            with torch.no_grad():
                tokens, gt_tags = batch
                # Assume batch_size=1
                tokens = [t[0] for t in tokens]
                gt_tags = [t[0] for t in gt_tags]
                tags = tagger.predict(tokens)
                predictions = [tt for tt in zip(tokens, tags)]
                groundtruths = [tt for tt in zip(tokens, gt_tags)]
    ...
    trainer = create_pt_trainer(model, loss_fn, optimizer, device)
